export default {
	getMatchQuery() {
		...
		let match_query = {
			started_at: { $gte: new Date(start_date.selectedDate).getTime(), $lte: new Date(end_date.selectedDate).getTime() } , success: true, visible: true
		};
		...
		if (user_id && user_id?.selectedOptionValue) {
			match_query.user_id = user_id.selectedOptionValue;
		}
